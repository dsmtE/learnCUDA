...
// ==================================================
		...
		if(idx < imgWidth && idy < imgHeight) {
			...
			for (uint j = 0; j < matSize; ++j ){
				for (uint i = 0; i < matSize; ++i ) {
					...
					sum.x += (float)tex2D(texRef, dX, dY) * matConv_cu_const[idMat];
					sum.y += (float)tex2D(texRef, dX, dY) * matConv_cu_const[idMat];
					sum.z += (float)tex2D(texRef, dX, dY) * matConv_cu_const[idMat];
				}
			}
    ...
    void studentJob(const std::vector<uchar4> &inputImg, // Input image
		...
		cudaMallocPitch(&input_cu, &pitch, imgWidth * 4 * sizeof(float), imgHeight * 4);
    	cudaMemcpy2D(input_cu, pitch, inputImg.data(), imgWidth * 4 * sizeof(float), imgWidth * 4 * sizeof(float), imgHeight * 4, cudaMemcpyHostToDevice);
