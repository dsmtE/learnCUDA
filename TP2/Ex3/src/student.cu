...
namespace IMAC {
		...
		if(idx < imgWidth && idy < imgHeight) {
			...
			for (uint j = 0; j < matSize; ++j ){
				for (uint i = 0; i < matSize; ++i ) {
					...
					sum.x += (float)tex1Dfetch(texRef, idPixel).x * matConv_cu_const[idMat];
					sum.y += (float)tex1Dfetch(texRef, idPixel).y * matConv_cu_const[idMat];
					sum.z += (float)tex1Dfetch(texRef, idPixel).z * matConv_cu_const[idMat];
				}
			}
    ...
    void studentJob(const std::vector<uchar4> &inputImg, // Input image
		...
		cudaBindTexture(nullptr, texRef, input_cu, inputImg.size() * sizeof(uchar4));
